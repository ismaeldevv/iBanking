...
const TransferAccountModal: React.FC<TransferAccountModalProps> = ({
  ...
  const [isNotIdOK, setIsNotIdOK] = useState<boolean>(false);
  const [isNotAmountOK, setIsNotAmountOK] = useState<boolean>(false);
  ...
  const onClick = (
    ...
    if (type === "close") return onClickTransfer("close");
    if (type === "transfer") {
      if (value.length > 1 && value2.length === 16) {
        onClickTransfer("transfer", Number(id), Number(account));
      } else {
        if (value2.length < 16) {
          setIsNotIdOK(true);
        } else {
          setIsNotAmountOK(false);
        }

        if (value.length < 1) {
          setIsNotAmountOK(true);
        } else {
          setIsNotAmountOK(false);
        }
      }
  ...
  return (
    <>
      <div className="bg-[rgba(0,0,0,0.4)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 absolute z-50 h-full w-full rounded-[0.5rem]"></div>
      <div
        role="dialog"
        id="radix-:r3:"
        ...
        <form className="flex flex-col gap-4">
          <div className="space-y-0.5">
            ...
            <div className="relative h-9">
              <input
                className="border-input ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring flex h-full w-full rounded-[0.345rem] border bg-transparent px-4 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-offset-0 disabled:cursor-not-allowed disabled:opacity-50"
                name="target"
                id=":ra5:-form-item"
                aria-describedby=":ra5:-form-item-description"
                aria-invalid="false"
                value={value2}
            ...
            {isNotIdOK && (
              <p className="text-[0.8rem] font-medium text-primary">
                Specify a correct id
              </p>
            )}
          </div>
          <div className="space-y-0.5">
            ...
            <div className="relative h-9">
              <input
                ...
                name="amount"
                id=":ra7:-form-item"
                aria-describedby=":ra7:-form-item-description"
                aria-invalid="false"
                value={value}
                onInput={handleInput}
              />
          ...
          <button
            ...
            type="button"
            onClick={() => onClick(type, value2, value)}
          >
            {label}
          </button>
        </form>
